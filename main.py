...
document.querySelectorAll('.upload-zone').forEach(zone=>{
  ...
  zone.addEventListener('drop',e=>{
    ...
    if(inp&&e.dataTransfer.files.length){
      // Simulate file selection for preview (can't set inp.files directly for folder)
      const label=zone.querySelector('.upload-label');
      if(label)label.textContent=e.dataTransfer.files.length+' file(s) dropped';
    }
  });
});
...
def _upload_widget(action_url, hint=""):
    """
...
<div class="upload-tab-group">
  ...
  <div class="upload-pane active" id="pane-files">
    <form method="POST" action="{action_url}" enctype="multipart/form-data" data-upload>
      <input type="hidden" name="upload_mode" value="files">
      <input type="hidden" name="_csrf_token" id="csrf-tok-files" value="">
  ...
  <div class="upload-pane" id="pane-folder">
    <form method="POST" action="{action_url}" enctype="multipart/form-data" data-upload>
      ...
      <input type="hidden" name="_csrf_token" id="csrf-tok-folder" value="">
      <div class="upload-zone">
  ...
  <div class="upload-pane" id="pane-zip">
    <form method="POST" action="{action_url}" enctype="multipart/form-data" data-upload>
      ...
      <input type="hidden" name="_csrf_token" id="csrf-tok-zip" value="">
      <div class="upload-zone">
...
def process_upload(files_field: list, dest_base: Path, allowed_ext: set):
    """
    Save uploaded files to dest_base.

    For folder uploads: browsers send each file with its webkitRelativePath
    embedded in the filename (e.g. "mysite/assets/style.css").
    We strip the first path component (the folder name the user picked),
    then recreate the subdirectory tree under dest_base.
    ...
    for f in files_field:
        ...
        # Folder upload: browser sets filename = "FolderName/sub/file.ext"
        if "/" in raw:
            parts=raw.split("/")
...
def build_web_server(config:Dict[str,Any],runtime_ref=None):
    ...
    def admin_index():
        ...
        body=(f'<div class="page-header"><div><div class="page-title">📁 File Manager</div>'
              ...
              f'</div></div>{table}</div>')
        return page("File Manager",body,"files")
    ...
    def admin_upload():
        ...
        if request.method=="POST":
            ...
            saved,errors=process_upload(flist,site_dir,_WEB_ALLOWED)
            if saved and not errors:msg=f"Uploaded {len(saved)} file(s).";mt="ok"
            elif saved:msg=f"Uploaded {len(saved)} | Errors: {'; '.join(errors[:3])}";mt="warn"
            else:msg="Failed: "+"; ".join(errors[:5]);mt="err"
            return _render_upload(msg,mt)
        return _render_upload()

    def _render_upload(flash_msg=None,flash_type=None):
        widget=_upload_widget("/admin/upload","HTML · CSS · JS · Images · Fonts · ZIP")
...
def build_storage_server(config:Dict[str,Any],runtime_ref=None):
    ...
    def _render(flash_msg=None,flash_type=None):
        ...
        widget=_upload_widget("/upload","All common file types · ZIP · Folder upload")
        stats=(f'<div class="stats-row">'
    ...
    def upload():
        ...
        flist=request.files.getlist("files")
        saved,errors=process_upload(flist,files_dir,_STORAGE_ALLOWED)
...
def build_bot_server(config:Dict[str,Any],runtime_ref=None):
    ...
    def admin_upload():
        ...
        if request.method=="POST":
            if not _csrf_validate(app.secret_key):
                return _render_upload("Request validation failed. Refresh and try again.","err"),403
            flist=request.files.getlist("files")
            saved,errors=process_upload(flist,bot_dir,_BOT_ALLOWED)
    ...
    def _render_upload(flash_msg=None,flash_type=None):
        widget=_upload_widget("/admin/upload",".py · .json · .yaml · .env · .txt · ZIP · Folder")
    ...
    def admin_env():
        ...
        rows="".join(
            f'<div class="env-row">'
            f'<input type="text" name="env_key[]" value="{k}" placeholder="KEY" style="margin:0">'
            f'<input type="text" name="env_val[]" value="{v}" placeholder="value" style="margin:0">'
        ...
        body=(f'<div class="page-header"><div class="page-title">🔑 Tokens &amp; Env Vars</div></div>'
              ...
              f'<form method="POST"><div id="env-container">{rows}</div>'
