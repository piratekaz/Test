...
def check_flask():
    ...
    try:
        __import__("flask");__import__("flask_sqlalchemy");ok("Flask + Flask-SQLAlchemy ✓")
    except ImportError:
        warn("Flask not found — installing…");install_package("flask");install_package("flask-sqlalchemy")
def check_werkzeug():
    try:__import__("werkzeug");ok("Werkzeug ✓")
...
def build_web_server(config:Dict[str,Any],runtime_ref=None):
    ...
    def admin_hosting():
        ...
        pub_cell='<a href="/admin/tunnel" class="btn btn-o btn-sm">Start →</a>' if not pu else ""
...
def build_bot_server(config:Dict[str,Any],runtime_ref=None):
    from flask import Flask,request,redirect,session,send_file as sf
    ...
    def admin_dash():
        ...
        controls=(
            ...
            +('<a href="/admin/bot/stop" class="btn btn-d">⏹ Stop Bot</a>'
               ...
               else '<a href="/admin/bot/start" class="btn btn-g">▶ Start Bot</a>')
            +'</div>'
        )
    ...
    def _render_upload(flash_msg=None,flash_type=None):
        ...
        for f in files:
            ...
            row_parts.append(
                ...
                f'<td class="fmeta">{format_size(f["size"])}</td>'
                f'<td class="fmeta">{f["modified"]}</td>'
                f'<td style="display:flex;gap:.35rem">'
                f'<a href="/admin/download/{f["relpath"]}" class="btn btn-w btn-sm" title="Download">⬇</a>'
                f'<a href="/admin/delete/{f["relpath"]}" class="btn btn-d btn-sm" {dc}>🗑</a></td></tr>'
            )
        rows="".join(row_parts)
    ...
    @app.route("/admin/download/<path:relpath>")
    def admin_download(relpath):
        g=gate()
        if g:return g
        t=(bot_dir/relpath).resolve()
        if not prevent_path_traversal(bot_dir,t) or not t.is_file():
            return redirect("/admin/upload")
        mime=mimetypes.guess_type(str(t))[0] or "application/octet-stream"
        return sf(str(t),as_attachment=True,mimetype=mime)
    ...
    def admin_deps():
        ...
        missing_html=(f'<div class="alert aw">⚠ Suggested for {bot_type}: '
                      ...
                      +' — <a href="/admin/deps/add-defaults" style="color:var(--accent)">Add all →</a></div>'
                      if missing else "")
        rows="".join(
        ...
        if plat=="linux":
            ...
            try:
                ...
                with tarfile.open(str(tgz)) as tf:
                    # Guard against tar slip: only extract safe members
                    safe=[m for m in tf.getmembers()
                          if not os.path.isabs(m.name) and ".." not in m.name]
                    tf.extractall(str(dest_dir),members=safe)
